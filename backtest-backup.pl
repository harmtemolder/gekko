...
# TODO Make all file and dir pointers absolute
...
our (
    ...
    @warmup,                        $from,
    $to,                            $csv,
    $print_roundtrips,              $threads,
    $stratsettings,                 $keep_logs,
    $asset_c,                       $currency_c,
    $fee_maker,                     $fee_taker,
    $fee_using,                     $slippage,
    $riskFreeReturn,                $use_toml_files,
    $toml_directory,                $note,
    $tocsv,                         $csv_columns,
    $debug,                         $cmc_data,
    $top_strategy_sort1,            $top_strategy_sort2,
    $all_results_sort,              $top_dataset_sort1,
    $top_dataset_sort2,             $print_all_results,
    $summary_interval,              $print_top_strategy,
    $print_top_dataset,             $print_analysis_on_summary,
    $all_results_limit,             $top_strategy_limit,
    $top_dataset_limit,             $dbname,
    $dbhost,                        $dbport,
    $dbuser,                        $dbpassword,
    $database,                      $default_set,
    $stfu,                          $all_results_min_profit,
    $all_results_min_profit_market, $all_results_min_profit_day,
    $all_results_min_trades_day,    $all_results_max_trades_day,
    $all_results_min_hodl_time,     $all_results_max_hodl_time
);
my (
    $oconfigfile,        $ostrat,               $opairs,
    $owarmup,            $oimport,              $operiod,
    $opaper,             $ohelp,                $oconvert,
    $ofrom,              $oto,                  $ocsv,
    %datasets,           $json,                 $gconfig,
    $currency,           $asset,                $exchange,
    $strategy,           $profit,               $profit_day,
    $profit_year,        $sharpe_ratio,         $market_change,
    $profit_market,      $trades,               $trades_day,
    $winning_trades,     $lost_trades,          $percentage_wins,
    $best_win,           $median_wins,          $worst_loss,
    $median_losses,      $avg_exposed_duration, $candlesize,
    $historysize,        $dataset_days,         $backtest_start,
    $dataset_from,       $dataset_to,           $price_volality,
    $volume,             $volume_day,           $overall_trades,
    $overall_trades_day, $template,             $oanalyze,
    $oinfo,              $dbh
);
my ( $open_price, $close_price, $highest_price, $lowest_price, $avg_price );

# Set directory in which gekko.js and its folders (e.g. history) reside
my $gekko_dir = "/Users/harmtemolder/stack/Github/gekko";

# Set directory in which backtest.pl and its config file reside
my $working_dir = $gekko_dir;
my $configfile  = $working_dir . "/backtest-config.pl";

# Set temporary directory to hold ... TODO fill in the blanks
mkdir $working_dir . '/tmp' unless -d $working_dir . '/tmp';
my $tmpfile = $working_dir . '/tmp';
...
GetOptions(
  ...
  or die
"Example: $0 --import --pair=bitfinex:USD:BTC --from=\"2018-01-01 09:10\" --to=\"2018-01-05 12:23\"\nUse backtest.pl --help for more details.\n";
...
sub dbcon {
    if ( $database eq 'sqlite' ) {
        $dbh = DBI->connect( "dbi:SQLite:dbname=$_[1]" . "_0.1.db", "", "", )
          or die $DBI::errstr;
    }
    elsif ( $database eq 'postgresql' ) {
        $dbh = DBI->connect(
            "dbi:pg:$dbname;host=$dbhost;port=$dbport;, $dbuser, $dbpassword");
    }
    elsif ( $_[0] eq 'csv' ) {
    }
    else {
...
sub analyze {
    if ( $print_all_results eq 'yes' ) {
        print "\n\n["
          ...
          . "] Creating ALL RESULTS table (sorted profit/day)...\n"
          if $stfu eq 'no';
        $dbh = DBI->connect( "dbi:CSV:", "", "",
            { RaiseError => 1, PrintError => 1, csv_eol => "\n" } );
        my $query =
"SELECT currency, asset, strategy, profit___, profit_market, profit_day___, trades_day, best_win, worst_loss, avg_HODL_min_, avg_price, overall_trades_day, volume_day FROM $_[0]";
        my $stc = $dbh->prepare($query);
        $stc->execute();
        my @all_results;
        while ( my @row = $stc->fetchrow_array ) {
            if (   $row[3] >= $all_results_min_profit
                && $row[4] >= $all_results_min_profit_market
                && $row[5] >= $all_results_min_profit_day
                && $row[6] >= $all_results_min_trades_day
                && $row[6] <= $all_results_max_trades_day
                && $row[9] >= $all_results_min_hodl_time
                && $row[9] <= $all_results_max_hodl_time )
            {
                push @all_results, [@row];
            }
        }
        my $table = Text::Table->new(
            { align_title => 'left', align => 'left', title => "Curr.\n----" },
    ...
    if ( $print_top_strategy eq 'yes' ) {
        ...
        while ( my @row = $stc->fetchrow_array ) {
            ...
            $query =
"SELECT AVG(avg_HODL_min_), AVG(profit___), MAX(profit___), MIN(profit___), AVG(trades_day), AVG(wins___), SUM(profit___), MAX(best_win), AVG(median_wins), MIN(worst_loss), AVG(median_losses) FROM $_[0] WHERE strategy='$row[0]' AND strategy_settings='$row[1]' AND trades_day > 0";
            my $stc = $dbh->prepare($query);
            $stc->execute();
            while ( my @row2 = $stc->fetchrow_array ) {
                $top_strategies{"$stratdata"}{hodl_time} =
                  sprintf( "%d", $row2[0] );
                $top_strategies{"$stratdata"}{avg_profit} =
                  sprintf( "%.2f", $row2[1] );
                $top_strategies{"$stratdata"}{worst_PL} =
                  sprintf( "%.1f", $row2[3] );
                $top_strategies{"$stratdata"}{best_PL} =
                  sprintf( "%.1f", $row2[2] );
                $top_strategies{"$stratdata"}{trades_day} =
                  sprintf( "%.1f", $row2[4] );
                $top_strategies{"$stratdata"}{trades_win} =
                  sprintf( "%.1f", $row2[5] );
                $top_strategies{"$stratdata"}{profits_sum} =
                  sprintf( "%d", $row2[6] );
                $top_strategies{"$stratdata"}{best_win_trade} =
                  sprintf( "%.2f", $row2[7] );
                $top_strategies{"$stratdata"}{avg_win_trade} =
                  sprintf( "%.2f", $row2[8] );
                $top_strategies{"$stratdata"}{worst_loss_trade} =
                  sprintf( "%.2f", $row2[9] );
                $top_strategies{"$stratdata"}{avg_loss_trade} =
                  sprintf( "%.2f", $row2[10] );

                #}
            }
            $query =
"SELECT profit___, profit_market  FROM $_[0] WHERE strategy='$row[0]' AND strategy_settings='$row[1]'";
            my $stc = $dbh->prepare($query);
            $stc->execute();
            my $counter = 1;
            while ( my @row2 = $stc->fetchrow_array ) {
            ...
            $top_strategies{"$stratdata"}{profitable} = sprintf(
                "%d",
                (
                    $top_strategies{"$stratdata"}->{profitable} * 100 / $counter
                )
            );
            $top_strategies{"$stratdata"}{profit_above_market} = sprintf(
                "%d",
                (
                    $top_strategies{"$stratdata"}->{profit_above_market} * 100 /
                      $counter
                )
            );
        }
        $query =
          qq(SELECT DISTINCT currency, asset, dataset_from, to FROM $_[0]);
        my $stc = $dbh->prepare($query);
        $stc->execute();
        while ( my @row = $stc->fetchrow_array ) {
            $query =
"SELECT strategy, profit_day___, strategy_settings FROM $_[0] WHERE currency='$row[0]' AND asset='$row[1]' AND dataset_from='$row[2]' AND to='$row[3]' ORDER by profit_day___ desc limit 1";
            my $stc = $dbh->prepare($query);
            $stc->execute();
            while ( my @row = $stc->fetchrow_array ) {
        ...
        for my $keys (
            sort {
                $top_strategies{$b}{$top_strategy_sort1}
                  <=> $top_strategies{$a}{$top_strategy_sort1}
                  or $top_strategies{$b}{$top_strategy_sort2}
                  <=> $top_strategies{$a}{$top_strategy_sort2}
            } keys %top_strategies
          )
        {
            ...
            $table->add(
                $strathuman[0],
                $top_strategies{$keys}->{best},
                $top_strategies{$keys}->{profitable},
                $top_strategies{$keys}->{profit_above_market},
                $top_strategies{$keys}->{best_PL},
                $top_strategies{$keys}->{worst_PL},
                $top_strategies{$keys}->{profits_sum},
                $top_strategies{$keys}->{avg_profit},
                $top_strategies{$keys}->{trades_win},
                $top_strategies{"$keys"}->{best_win_trade},
                $top_strategies{"$keys"}->{avg_win_trade},
                $top_strategies{"$keys"}->{worst_loss_trade},
                $top_strategies{"$keys"}->{avg_loss_trade},
                $top_strategies{$keys}->{trades_day},
                $top_strategies{$keys}->{hodl_time}
            );
    ...
    if ( $print_top_dataset eq 'yes' ) {
        ...
        print "["
          . strftime( "%Y-%m-%d %H:%M:%S", localtime )
          . "] Creating TOP DATASET table...\n"
          if $stfu eq 'no';
        my %top_dataset;
        my $pairdata;
        my $query =
          "SELECT DISTINCT currency, asset, dataset_from, to FROM $_[0]";
        my $stc = $dbh->prepare($query);
        $stc->execute();
        while ( my @row = $stc->fetchrow_array ) {
            ...
            $query =
"SELECT AVG(avg_HODL_min_), AVG(profit___), MAX(profit___), MIN(profit___), AVG(trades_day), AVG(wins___), SUM(profit___) FROM $_[0] WHERE currency='$row[0]' AND asset='$row[1]' AND dataset_from='$row[2]' AND to='$row[3]' ";
            my $stc = $dbh->prepare($query);
            $stc->execute();
            while ( my @row2 = $stc->fetchrow_array ) {
                $top_dataset{$pairdata}{hodl_time} = sprintf( "%d", $row2[0] );
                $top_dataset{$pairdata}{avg_profit} =
                  sprintf( "%.1f", $row2[1] );
                $top_dataset{$pairdata}{worst_PL} = sprintf( "%.1f", $row2[3] );
                $top_dataset{$pairdata}{best_PL}  = sprintf( "%.1f", $row2[2] );
                $top_dataset{$pairdata}{trades_day} =
                  sprintf( "%.1f", $row2[4] );
                $top_dataset{$pairdata}{trades_win} =
                  sprintf( "%.1f", $row2[5] );
                $top_dataset{$pairdata}{profits_sum} =
                  sprintf( "%d", $row2[6] );
            }
            $query =
"SELECT profit___, profit_market, volatility, cur_CMC_rank, cur_marketcap, cur_CMC_volume, days, market_change___ FROM $_[0] WHERE currency='$row[0]' AND asset='$row[1]' AND dataset_from='$row[2]' AND to='$row[3]' ";
            ...
            if ( $counter > 0 ) {
                $top_dataset{$pairdata}{profitable} = sprintf( "%d",
                    ( $top_dataset{$pairdata}->{profitable} * 100 / $counter )
                );
                $top_dataset{$pairdata}{profit_above_market} = sprintf(
                    "%d",
                    (
                        $top_dataset{$pairdata}->{profit_above_market} * 100 /
                          $counter
                    )
                );
            }
        ...
        while ( my @row = $stc->fetchrow_array ) {
            $query =
"SELECT currency, asset, profit_day___, dataset_from, to FROM $_[0] WHERE strategy='$row[0]' AND strategy_settings='$row[1]' ORDER by profit_day___ desc limit 1";
            my $stc = $dbh->prepare($query);
            $stc->execute();
            while ( my @row = $stc->fetchrow_array ) {
        ...
        for my $keys (
            sort {
                $top_dataset{$b}{$top_dataset_sort1}
                  <=> $top_dataset{$a}{$top_dataset_sort1}
                  or $top_dataset{$b}{$top_dataset_sort2}
                  <=> $top_dataset{$a}{$top_dataset_sort2}
            } keys %top_dataset
          )
        {
            ...
            $table->add(
                $pairhuman[0],
                $top_dataset{$keys}->{best},
                $top_dataset{$keys}->{profitable},
                $top_dataset{$keys}->{profit_above_market},
                $top_dataset{$keys}->{market_change},
                $top_dataset{$keys}->{best_PL},
                $top_dataset{$keys}->{worst_PL},
                $top_dataset{$keys}->{profits_sum},
                $top_dataset{$keys}->{avg_profit},
                $top_dataset{$keys}->{trades_win},
                $top_dataset{$keys}->{trades_day},
                $top_dataset{$keys}->{hodl_time},
                $top_dataset{$keys}->{price_volatility},
                $top_dataset{$keys}->{cmc_rank},
                $top_dataset{$keys}->{cmc_marketcap},
                $top_dataset{$keys}->{cmc_volume},
                $top_dataset{$keys}->{days}
            );
...
# Overwrite all default variables with any given as command parameters
if ($oconfigfile) {
    $configfile = $oconfigfile;
}
require $configfile;    # Moved here because of --config without default file
...
if ($oinfo) {
    my $dbh = DBI->connect(
        "dbi:SQLite:dbname=history/binance_0.1.db",
        "", "", { RaiseError => 0 },
    );
    my $stmt =
qq(SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%candles%';);
    my $sth = $dbh->prepare($stmt);
    my $rv  = $sth->execute();
    my @row;
    while ( @row = $sth->fetchrow_array() ) {
        $stmt =
qq(SELECT datetime(start, 'unixepoch') FROM `main`.@row order by start ASC LIMIT 1;);
        my $sth = $dbh->prepare($stmt);
        my $rv  = $sth->execute();
        my @row2;
        ...
        $stmt =
qq(SELECT datetime(start, 'unixepoch') FROM `main`.@row order by start DESC LIMIT 1;);
        my $sth = $dbh->prepare($stmt);
        my $rv  = $sth->execute();
        my @row2;
        while ( @row2 = $sth->fetchrow_array() ) {
...
if ($oimport) {
    ...
    if ( grep /binance|bitfinex|coinfalcon|kraken/, @pairs ) {
        ...
        @match = uniq @match;
        {
            local $CWD = $working_dir . 'exchange/util/genMarketFiles';
            foreach (@match) {
        ...
        foreach (@pairs) {
            ...
            if ( $sets[1] eq 'ALL' || $sets[2] eq 'ALL' ) {
                my $filename =
                  $working_dir . "exchange/wrappers/$sets[0]-markets.json";
                my $json_text = do {
                ...
                for my $i ( 0 .. $rec ) {
                    push @pairs,
"$sets[0]:$data->{markets}->[$i]->{pair}->[0]:$data->{markets}->[$i]->{pair}->[1]";
                }
                if ( $sets[2] eq 'ALL' ) {
    ...
    if ( grep !/binance|bitfinex|coinfalcon|kraken/,
        @pairs && grep /ALL/, @pairs )
    {
        print
"Value ALL is allowed only for binance, bitfinex, coinfalcon, kraken exchanges. You must add your pairs manually.\n";
        exit;
    }
    ...
    foreach (@pairs) {
        ...
        if ( $ofrom eq 'last' ) {
            ...
            my $dbh       = DBI->connect(
                "dbi:SQLite:dbname=" . $working_dir . "history/$sets[0]_0.1.db",
                "", "", { RaiseError => 1 },
            ) or die $DBI::errstr;
            if ( $sets[1] eq 'ALL' || $sets[2] eq 'ALL' ) {
                my $stmt =
qq[CREATE TABLE IF NOT EXISTS \'$tablename\' (id INTEGER PRIMARY KEY AUTOINCREMENT, start INTEGER UNIQUE, open REAL NOT NULL, high REAL NOT NULL, low REAL NOT NULL, close REAL NOT NULL, vwp REAL NOT NULL, volume REAL NOT NULL, trades INTEGER NOT NULL);];
                my $sth = $dbh->prepare($stmt);
            ...
            my $stmt =
qq{SELECT start FROM \'$tablename\' ORDER BY start DESC LIMIT 0,1;};
            my $sth = $dbh->prepare($stmt);
            my $rv = $sth->execute() or die $DBI::errstr;
            if ( $rv < 0 ) {
                print $DBI::errstr;
...
s/(?<=batchSize: 50\n)(.*)(?=\n)/}\nconfig.importer = {\n  daterange: {\n    from: \"$from\",\n    to: \"$to\"\n  }\n}/m;
        ...
        $gconfig =~
          s/(?<=config.performanceAnalyzer = \{\n  enabled: )(.*)(?=,)/false/m;
        $gconfig =~
          s/(?<=config.tradingAdvisor = \{\n  enabled: )(.*)(?=,)/false/m;
        $gconfig =~ s/(?<=config.trader = \{\n  enabled: )(.*)(?=,)/false/m;
        $gconfig =~
          s/(?<=config.paperTrader = \{\n  enabled: )(.*)(?=,)/false/m;
        $gconfig =~
          s/(?<=config.candleWriter = \{\n  enabled: )(.*)(?=,)/true/m;
        ...
        $configfile->flush;
        if ( $keep_logs eq 'yes' ) {
            open my $fh2, '>>:encoding(UTF-8)', $logfile
        ...
        if ( $grun !~ m/Done importing/ ) {
            ...
            print
"Import of $sets[0] $sets[1]-$sets[2] is done. Elapsed time: $elapsedthread. \n"
              . --$remain
              . " from "
              . scalar @pairs
              . " pairs left.\n";
    ...
    if ($errcheck) {
        print "Had $errcheck errors on "
          . scalar @pairs
          . " jobs. Elapsed time: $elapsedapp\n";
    }
    else {
...
if ($opaper) {
    ...
    foreach (@warmup) {
        ...
        foreach (@pairs) {
            ...
            foreach (@strategies) {
                my $configfile =
                  "$sets[1]-$sets[2]-$_-$warms[0]-$warms[1]-config.js";

                # Log file name.
                my $logfile =
                  "logs/paper-$sets[1]-$sets[2]-$_-$warms[0]-$warms[1].log";
                $gconfig =~
s/(?<=config.candleWriter = \{\n  enabled: )(.*)(?=\n)/false/m;
                $gconfig =~ s/(?<=exchange: ')(.*?)(?=',)/$sets[0]/g;
                $gconfig =~ s/(?<=currency: ')(.*?)(?=',)/$sets[1]/g;
                ...
                open my $fh8, '>:encoding(UTF-8)', $configfile
                  or die "Cannot open $configfile!";
                print $fh8 join( "\n", $gconfig );
                close $fh8;
...
#my $tmpcsv = File::Temp->new(TEMPLATE => "tmp_dataXXXXX",SUFFIX => ".csv",DIR => $tmpfile);
    ...
    if ( !-e $csv ) {
        ...
        sub columns {
            ...
            open my $fh3, '>>:encoding(UTF-8)', $working_dir . $csv
              or die "Cannot open " . $working_dir . "$csv!";
            print $fh3 join( "\n", "$out\n" );
            close $fh3;
        }
        &columns($csv_columns);
    ...
    if ( $cmc_data eq 'yes' ) {
        ...
        $req->timeout(10);
        my $response =
          $req->get("https://api.coinmarketcap.com/v1/ticker/?limit=800");
        if ( $response->is_success ) {
            ...
            %cmcvolume =
              map { $_->{symbol} => $_->{'24h_volume_usd'} } @$response;
            %cmcrank = map { $_->{symbol} => $_->{'rank'} } @$response;
            %cmcmarketcap =
              map { $_->{symbol} => $_->{'market_cap_usd'} } @$response;
        }
        else {
    ...
    if ( grep /ALL/, @pairs || grep /ALL/, @strategies ) {
        print "["
          . strftime( "%Y-%m-%d %H:%M:%S", localtime )
          . "] Parameter ALL detected. Prepare data\n"
          if $stfu eq 'no';
    }
    if ( grep /^ALL$/, @pairs ) {
...
qq(SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%candles%';);
                ...
                $stmt =
qq(SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%candles_$sets[1]%';);
            }
            my $sth = $dbh->prepare($stmt);
            my $rv = $sth->execute() or die $DBI::errstr;
            if ( $rv < 0 ) {
    ...
    if ( $cmc_data eq 'yes' ) {
        ...
        foreach (@pairs) {
            ...
            if ( grep /$cmcasset/, %cmcvolume ) {
                $paircalc{"cmcvol$pairname"} =
                  sprintf( "%d", $cmcvolume{$cmcasset} );
                $paircalc{"cmcrank$pairname"} = $cmcrank{$cmcasset};
                $paircalc{"cmccurmarketcap$pairname"} =
                  sprintf( "%d", $cmcmarketcap{$cmcasset} );
            }
        }
    }
    my %stratconfig;
...
/(?:(?:[\d-\.]+[, ]+)+[\d-\.]+)|([-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*)/
                  )
                {
                    ...
                    print "["
                      . strftime( "%Y-%m-%d %H:%M:%S", localtime )
                      . "] Generate params combinations for: $strat\n"
                      if $stfu eq 'no';
                    if ( $substitute =~
                        /(?<!")\b(?:(?:[\d-\.]+[, ]+)+[\d-\.]+)\b(?!")/ )
                    {
            ...
            if (   $stratconfig{$strat} =~ /[-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*/
                && $stratconfig{$strat} =~ /(?:(?:[\d-\.]+[, ]+)+[\d-\.]+)/ )
            {
                my @match1 =
                  $stratconfig{$strat} =~ /[-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*/g;
                my @match2 =
                  $stratconfig{$strat} =~ /(?:(?:[\d-\.]+[, ]+)+[\d-\.]+)/sg;
                my ( @aoa2, @tuples1, $b1 );
                foreach (@match2) {
                    $_ =~ s/ //g;
                    my @bfval = split /\,/, $_;
                ...
                my (
                    @arraytmp,      @stratconfcalues, @matchcyfra,
                    @stratconfcomb, $stratconffin
                );
                foreach (@match1) {
                    @arraytmp = ();
                    @matchcyfra = split /\.\.|:/, $_;
                    if ( $matchcyfra[0] > $matchcyfra[1] ) {
                        ( $matchcyfra[0], $matchcyfra[1] ) =
                          ( $matchcyfra[1], $matchcyfra[0] );
                    }
                    if ( ( $matchcyfra[1] - $matchcyfra[0] ) < $matchcyfra[2] )
                    {
                        print
"Too few possibilities from the expression: $matchcyfra[0]..$matchcyfra[1]:$matchcyfra[2]. Try extend range by first (range start) and/or second (range end) value and/or reduce step - third value. \n\nEg: expression 0..10:2 generate 0, 2, 4, 6, 8, 10 values for strat parameter.\n";
                        exit;
                    }
                    for (
                        my $i = $matchcyfra[0] ;
                ...
                foreach my $taba2 (@tuples2) {
                    ...
                    if ( $b2 > 1 ) {
                        for ( my $i = 0 ; $i <= ( $b2 - 1 ) ; $i++ ) {
                            ...
                            $s =~
                              s/[-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*/$stratconffin/;
                        }
                        my $ddd =
                          $strat . ' ' . join( '|', map { "$_" } @{$taba2} );
            ...
            if ( $stratconfig{$strat} =~ /[-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*/ ) {
                my @match =
                  $stratconfig{$strat} =~ /[-\d+\.]*\.\.[-\d+\.]*:[-\d+\.]*/g;
                my ( @arraytmp, @stratconfcalues, @matchcyfra, @stratconfcomb,
                    $stratconffin, @tuples );
                foreach (@match) {
                    @arraytmp = ();
                    @matchcyfra = split /\.\.|:/, $_;
...
"Too few possibilities from the expression: $matchcyfra[0]..$matchcyfra[1]:$matchcyfra[2]. Try extend range by first (range start) and/or second (range end) value and/or reduce step - third value. \n\nEg: expression 0..10:2 generate 0, 2, 4, 6, 8, 10 values for strat parameter.\n";
            ...
            if ( $stratconfig{$strat} =~ /#E#/ ) {
                ...
                print "["
                  . strftime( "%Y-%m-%d %H:%M:%S", localtime ) . "] "
                  . scalar @configs
                  . " configuration found for strategy $strat\n";
                for my $i ( 1 .. $#configs ) {
    ...
    if ( $use_toml_files eq 'yes' ) {
        print "["
          . strftime( "%Y-%m-%d %H:%M:%S", localtime )
          . "] Use TOML files for strategies configuration\n"
          if $stfu eq 'no';
    }
    if ( $remain > 1000 && $print_analysis_on_summary eq 'yes' ) {
        print "["
          . strftime( "%Y-%m-%d %H:%M:%S", localtime )
          . "] WARNING: Printing a table during so many backtests may degrade performance. It is recommended to disable \n"
          . '$print_analysis_on_summary '
          . " for the duration of such a large test.\n";
    }
    print "["
      . strftime( "%Y-%m-%d %H:%M:%S", localtime )
    ...
    print "["
      . strftime( "%Y-%m-%d %H:%M:%S", localtime )
      . "] Starting Backtest Machine ($threads threads)...
============================================================== I N P U T    D A T A ===============================================================
Pairs
-----
$palist
...
============================================================= L A S T    R E S U L T S ============================================================\n"
    ...
    if ( scalar @warmup > 1 ) {
        $outformat =
          "%-9s %+40s %+12s %+15s %+11s %+11s %+9s %+11s %+7s %+5s %+6s";
        printf $outformat, "Pair", "Strategy", "Profit", "Profit-market",
          "Trades/day", "Win trades", "Best win", "Worst loss", "Candle",
          "Days", "Time";
        $columns_end =
"\n----                                      --------       ------   -------------  ----------  ----------  --------  ----------  ------  ----   ----\n";
    }
        ...
        $outformat =
          "%-9s %+40s %+12s %+15s %+11s %+11s %+9s %+11s %+7s %+5s %+6s";
        printf $outformat, "Pair", "Strategy", "Profit", "Profit-market",
          "Trades/day", "Win trades", "Best win", "Worst loss", "HODL", "Days",
          "Time";
        $columns_end =
"\n----                                      --------       ------   -------------  ----------  ----------  --------  ----------    ----  ----   ----\n";
    }
    ...
    foreach (@warmup) {
        if (   @warmup >= scalar( keys %stratconfig )
            && @warmup >= @pairs
            && @warmup > 1 )
        {
        ...
        my @warms = split /:/, $_;
        foreach (@pairs) {
            if (   @pairs > scalar( keys %stratconfig )
                && @pairs >= @warmup
                && @pairs > 1 || $bftriger )
            {
                $pid = $pm->start and next;
...
#print "\n\n+++++++++++++++++++++++++++++++++\nDEBUG\nStrat name: $stratn \nConfig: \n$stratconfig{$stratn}\n++++++++++++++++++++++++++++++++++++++++++\n\n";
                if (   scalar( keys %stratconfig ) >= @pairs
                    && scalar( keys %stratconfig ) >= @warmup
                    && scalar( keys %stratconfig ) > 1
                    && !defined $bftriger )
                {
                    $pid = $pm->start and next;
                }
                ...
                # Log file name.
                my $logfile =
                  "logs/$sets[1]-$sets[2]-$stratn-$warms[0]-$warms[1].log";
                $gconfig = $gconfig1;
                my $startthread = time;
                $configfile = File::Temp->new(
                    TEMPLATE => "tmp_configXXXXX",
                    SUFFIX   => ".js",
                    DIR      => $tmpfile
                );
...
qq{SELECT start FROM \'$tablename\' ORDER BY start DESC LIMIT 0,1;};
                    ...
                    if ( !defined $to ) {
                        print
"Haven't datasets in this period for $sets[0]:$sets[1]:$sets[2]\n";
                    }
                    else {
                        $from = strftime "%Y-%m-%d %H:%M:%S",
                          gmtime( $to - $operiod * 86400 );
...
s/(?<=config.candleWriter = \{\n  enabled: )(.*)(?=\n)/false/m;
                $gconfig =~
s/(simulationBalance: \{)\s+asset:\s+\d,\s+currency:\s\d,/simulationBalance: \{\n    asset: $asset_c,\n    currency: $currency_c,/g;
                my $stratcvs = 'NA';
...
s/(?<=config.candleWriter = \{\n  enabled: )(.*)(?=\n)/false/m;
                ...
                $gconfig =~ s/(?<=candleSize: )(.*?)(?=,)/$warms[0]/g;
                $gconfig =~ s/(?<=historySize: )(.*?)(?=,)/$warms[1]/g;
                open my $fh15, '>:encoding(UTF-8)', $configfile
                  or die "Cannot open $configfile!";
                print $fh15 join( "\n", $gconfig );
                close $fh15;
                ...
                my @profit = $grun =~
/(?<=simulated profit:\t\t )[0-9.\-][0-9.\-]* $sets[1] \((.*)(?=\%\))/;
                my @yearly = $grun =~
/(?<=simulated yearly profit:\t )[0-9.\-][0-9.\-]* $sets[1] \((.*)(?=\%\))/;
                my @trades     = $grun =~ /(?<=trades:\t\t )(.*?)(?=\n)/;
                my @period     = $grun =~ /(?<=timespan:\t\t\t )(.*?)(?=\n)/;
                my @start      = $grun =~ /start time:\s+\K(.*)/;
                my @end        = $grun =~ /end time:\s+\K(.*)/;
                my @roundtrips = $grun =~
                  /ROUNDTRIP\)\s+\d{4}-\d{2}-\d{2}.*?-?\d+\.\d+\s+\K-?\d+.\d+/g;
                ...
                if ( @roundtrips > 0 ) {
                    $percentage_wins =
                      sprintf( "%.2f", @wins * 100 / @roundtrips );
                }
                $median_wins = median @wins;
                $median_wins =~ s/,/\./;
                ...
                foreach (@exposed) {
                    my @exposed = split /\t/, $_;
                    my $roznica =
                      ( &toepoch( $exposed[1] ) - &toepoch( $exposed[0] ) ) /
                      60;
                ...
                if ( defined $end[0] && $start[0] ) {
                    $dataset_days = sprintf( "%d",
                        ( &toepoch( $end[0] ) - &toepoch( $start[0] ) ) /
                          86400 );
                    if ( $dataset_days > 0 ) {
                        $trades_day =
                          sprintf( "%.2f", $trades[0] / $dataset_days );
                        $profit_day =
                          sprintf( "%.2f", $profit[0] / $dataset_days );
                    }
                    my @sharpe_ratio =
                      $grun =~ /\(PROFIT REPORT\) sharpe ratio:\s+\K.*/g;
                    ...
                    if ( !$paircalc{"rsd$pairname"} ) {
                        ...
                        my $stmt =
qq(SELECT high FROM candles_$dbtable WHERE start >= strftime('%s', '$start[0]') AND start <= strftime('%s', '$end[0]'));
                        my $sth = $dbh->prepare($stmt);
                        my $rv = $sth->execute() or die $DBI::errstr;
                        if ( $rv < 0 ) {
                            print $DBI::errstr;
                        ...
                        $stmt =
qq(SELECT low FROM candles_$dbtable WHERE start >= strftime('%s', '$start[0]') AND start <= strftime('%s', '$end[0]'));
                        ...
                        my $avg = avg(@ceny);
                        $price_volality =
                          sprintf( "%.1f", stddev(@ceny) * 100 / $avg );
                        $paircalc{"rsd$pairname"} = $price_volality;
                        my ( $var, $var2 );

                        sub sum {
                            $stmt =
qq(SELECT $_[0] FROM candles_$dbtable WHERE start >= strftime('%s', '$start[0]') AND start <= strftime('%s', '$end[0]'))
                              . "$_[1];";
                            $sth = $dbh->prepare($stmt);
                            $rv = $sth->execute() or die $DBI::errstr;
                            if ( $rv < 0 ) {
                    ...
                    open my $fh4, '>>:encoding(UTF-8)', $working_dir . $csv
                      or die "Cannot open " . $working_dir . "$csv!";
                    print $fh4 join( "\n", "$csvout\n" );
                    close $fh4;
                    $tocsvtmp = Template->new( {} );
                    $tocsvtmp->process( $csv_columns_tmp, $vars, \$csvouttmp );
                    open my $fh5, '>>:encoding(UTF-8)', $working_dir . $tmpcsv
                      or die "Cannot open " . $working_dir . "$tmpcsv!";
                    print $fh5 join( "\n", "$csvouttmp\n" );
                    close $fh5;
                    ...
                    my $endthread = time;
                    if ( scalar @warmup > 1 ) {
                        printf $outformat, "$sets[1]:$sets[2]", $stratn,
                          "$profit%", $profit_market, $trades_day,
                          "$percentage_wins%", "$best_win%", "$worst_loss%",
                          "$warms[0]:$warms[1]", $dataset_days,
                          strftime( "\%M:\%S",
                            gmtime( $endthread - $startthread ) );
                    }
                    else {
                        printf $outformat, "$sets[1]:$sets[2]", $stratn,
                          "$profit%", $profit_market, $trades_day,
                          "$percentage_wins%", "$best_win%", "$worst_loss%",
                          "$avg_exposed_duration", $dataset_days,
                          strftime( "\%M:\%S",
                            gmtime( $endthread - $startthread ) );
                    }
                    print "\n";
                ...
                if ( -e "backtest.dat" ) {
                    open( my $fh, '<:encoding(UTF-8)', "backtest.dat" )
                      or die "Could not open file '!";
                    my $row = <$fh>;
                    close $fh;
                    $row++;
                    open my $fh3, '>:encoding(UTF-8)',
                      $working_dir . 'backtest.dat'
                      or die "Cannot open " . $working_dir . "$csv!";
                    print $fh3 $row;
                    close $fh3;
                    my $key;
                    ...
                    if ( grep { $coun{$_} eq $row } keys %coun ) {
                        ...
                        $key = $rhash{$row};
                        print( "=" x 147 );
                        $print_analysis_on_summary eq 'yes'
                          ? &analyze($tmpcsv)
                          : print "\n\n";
                        print "["
                          . strftime( "%Y-%m-%d %H:%M:%S", localtime ) . "] ";
                        ...
                        print "% complete. Elapsed: "
                          . strftime( "%H:%M:%S",
                            gmtime( ( time - $startbacktesttime ) ) )
                          . ", average backtest duration: "
                          . strftime(
                            "%M:%S",
                            gmtime(
                                ( time - $startbacktesttime ) / $row * $threads
                            )
                          )
                          . ", backtests remain: "
                          . ( $remain - $row )
                          ...
                          . strftime(
                            "%H:%M:%S",
                            gmtime(
                                ( time - $startbacktesttime ) * 100 / $key -
                                  ( time - $startbacktesttime )
                            )
                          )
                          . "\n\n============================================================= L A S T    R E S U L T S ============================================================\n"
                          if $stfu eq 'no';
                        if ( scalar @warmup > 1 ) {
                            $outformat =
"%-9s %+40s %+12s %+15s %+11s %+11s %+9s %+11s %+7s %+5s %+6s";
                            printf $outformat, "Pair", "Strategy", "Profit",
                              "Profit-market", "Trades/day", "Win trades",
                              "Best win", "Worst loss", "Candle", "Days",
                              "Time";
                            $columns_end =
"\n----                                      --------       ------   -------------  ----------  ----------  --------  ----------  ------  ----   ----\n";
                        }
...
"%-9s %+40s %+12s %+15s %+11s %+11s %+9s %+11s %+7s %+5s %+6s";
                            printf $outformat, "Pair", "Strategy", "Profit",
                              "Profit-market", "Trades/day", "Win trades",
                              "Best win", "Worst loss", "HODL", "Days", "Time";
                            $columns_end =
"\n----                                      --------       ------   -------------  ----------  ----------  --------  ----------    ----  ----   ----\n";
                        }
                        print $columns_end;
    ...
    print "\n\n["
      . strftime( "%Y-%m-%d %H:%M:%S", localtime )
      . "] 100% complete. Elapsed: "
      . strftime( "%H:%M:%S", gmtime( ( time - $startbacktesttime ) ) )
      . ", average backtest duration: "
      . strftime(
        "%M:%S",
        gmtime(
            ( time - $startbacktesttime ) / $remain * ( $remain / $threads )
        )
      ) if $stfu eq 'no';
    print "\n";
    &analyze($tmpcsv);
